...
namespace MsgPassingCommunication
{
	Context::Context()
	{
		Context::saveFilePath = "C:/SU/temp/Project4Sample/CppCommWithFileXfer/SaveFiles";  // default staging area for incoming files
		Context::sendFilePath = "C:/SU/temp/Project4Sample/CppCommWithFileXfer/SendFiles";  // default staging area for outgoing files
	}
	Context ctx;
...
bool Sender::sendFile(Message msg)
{
	...
	for (auto msgValue : msg.attributes())
	{
		if (msgValue.first.find("sendingFile") != std::string::npos)
			fileNames.push_back(msgValue.second);
	}

	for (auto msgValue : fileNames)
	{
		//if (!msg.containsKey("sendingFile"))
		//	return false;
		std::string fileSpec = ctx.sendFilePath + "/" + msgValue;//msg.value("sendingFile");
		fileSpec = FileSystem::Path::getFullFileSpec(fileSpec);
		std::ifstream sendFile(fileSpec, std::ios::binary/*,_SH_DENYNO*/);
		msg.attribute("recieveFile", msgValue);
		if (!sendFile.good())
		{
			std::cout << "\n  can't open \"" << fileSpec << "\"";
			return false;
		...
		while (true)
		{
			...
			std::string countStr = Utilities::Converter<size_t>::toString(blockSize + BlockSize);
			if (msg.containsKey("verboseBytes"))
				msg.attribute("bytesTransferred", countStr);
...
public:
	...
	*/
	...
	{
		std::string msgValue = msg.value("recieveFile");
		std::string fileName = msgValue;// msg.value("sendingFile");
		...
		while (true)
		{
			if (msg.containsKey("verbose"))
			{
				std::cout << "\n  receiving file block from \"" << msgValue;//msg.value("sendingFile");
			}
