...
std::string Path::getRootFolder(std::string path)
{
	std::string pathNormalized = Path::getFullFileSpec(path);
	size_t startIndex = pathNormalized.find_first_of("\\");
	size_t endIndex = pathNormalized.substr(startIndex + 1).find_first_of("\\");
	return pathNormalized.substr(startIndex + 1, startIndex + endIndex - 2);
}
...
// using the path passed to this function
bool Directory::createDirectory(const std::string& path)
{
	bool creationError = true;
	std::string directoryPath = path.substr(0, path.find_last_of("\\"));
	if (creationError && !Directory::exists(directoryPath))
	{
		creationError = createDirectory(directoryPath);
