...
namespace NoSqlDb
{
	...
	std::ostream& operator<<(std::ostream& out, PayLoad payloadObj)
	{
		...
		std::string status;
		if (payloadObj.getCheckInStatus() == ApplicationConstants::CheckinStatus::Open)
			status = "Open";

		if (payloadObj.getCheckInStatus() == ApplicationConstants::CheckinStatus::Closed)
			status = "Closed";

		if (payloadObj.getCheckInStatus() == ApplicationConstants::CheckinStatus::ClosePending)
			status = "ClosePending";

		if (payloadObj.getCheckInStatus() == ApplicationConstants::CheckinStatus::New)
			status = "New";
		out << "|| Status: " << status;
		out << "|| User: " << payloadObj.getUser();
		return out;
	}

	std::string PayLoad::getPayLoadStatusStr(ApplicationConstants::CheckinStatus status)
	{
		std::string statusStr;
		if (status == ApplicationConstants::CheckinStatus::Open)
			statusStr = "Open";

		if (status == ApplicationConstants::CheckinStatus::Closed)
			statusStr = "Closed";

		if (status == ApplicationConstants::CheckinStatus::ClosePending)
			statusStr = "ClosePending";

		if (status == ApplicationConstants::CheckinStatus::New)
			statusStr = "New";
		return statusStr;
	}
