...
public:
	...
	std::regex& getVersionNo() { return version_; }
	std::regex& getChildrenFIlter() { return childrenRegex_; }
	

	Conditions& setNameRegex(const std::string& regex);
	Conditions& setCrenRegex(const std::string& regex);
	...
	Conditions& setVersion(const std::string& regex);
