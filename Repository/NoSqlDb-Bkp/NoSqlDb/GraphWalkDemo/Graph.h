...
namespace GraphProcessing
{
	...
	public:
		...
		void walker(T t, Sptr<V> pNode, P p, Z z, bool walkAll = false);
	private:
	...
	*/
	...
	template<typename V>
	template<typename T, typename P, typename Z>
	void Graph<V>::walker(T t, Sptr<V> pNode, P p, Z z, bool walkAll)
	{
		// traverse the graph, applying processing p to each node
		t(pNode, p, z);
	...
	template<typename V, typename P, typename Z>
	void DFS_r_ltr(Sptr<V> pNode, V NodeVal, P p, Z z)
	{
		if (pNode->marked())
		{
			z(pNode, gpNodeVal);
...
template<typename V, typename P, typename Z>
void BFS_q_marked(Sptr<V> pNode, P p, Z z)
{
	std::queue<Sptr<V>> nodeQueue;
	nodeQueue.push(pNode);
	bool execFlag = true;
